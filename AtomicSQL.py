...
log = logger.getLogger("AtomicSQL")
...
class AtomicSQL:
    ...
    def __init__(self):
        ...
        self.db = (
            {}
        )  # used to hold the database connections and commit changes and such
    ...
    def safe_execute(self, session_id, action):
        ...
        if session_id not in self.db:
            ValueError
            log.exit(f"Session ID {session_id!r} is invalid.")
        self.master_lock.acquire()
        ...
        try:
            ...
            while True:
                try:
                    ...
                    delay = 3 * failures
                    log.warning_(
                        f"AtomicSQL.safe_execute failed, retrying in {delay} seconds..."
                    )
                    time.sleep(delay)
                if failures == 10:
                    ValueError
                    log.exit("AtomicSQL.safe_execute failed too many time's. Aborting.")

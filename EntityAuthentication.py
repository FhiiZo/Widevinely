...
from widevinely.utils.MSL import EntityAuthenticationSchemes
from widevinely.utils.MSL.MSLObject import MSLObject
from widevinely.utils import logger

log = logger.getLogger("MSL.EntityAuthentication")
...
class EntityAuthentication(MSLObject):
    ...
    def UnauthenticatedSuffixed(cls, root: str, suffix: str) -> EntityAuthentication:
        ...
        return cls(
            ...
            authdata={"root": root, "suffix": suffix},
        )
    ...
    def Provisioned(cls) -> EntityAuthentication:
        ...
        """
        return cls(scheme=EntityAuthenticationSchemes.Provisioned, authdata={})

    @classmethod
    def PSKorMGK(
        cls, scheme: EntityAuthenticationSchemes, identity: str
        ...
        if not isinstance(scheme, EntityAuthenticationSchemes):
            ValueError
            log.exit("scheme must be an EntityAuthenticationScheme")
        if scheme not in [
            EntityAuthenticationSchemes.PreSharedKeys,
            EntityAuthenticationSchemes.ModelGroupKeys,
        ]:
            ValueError
            log.exit("scheme must be either PreSharedKeys or ModelGroupKeys")
        return cls(scheme=scheme, authdata={"identity": identity})
    ...
    def RSA(cls, identity: str, pubkeyid: str) -> EntityAuthentication:
        ...
        return cls(
            ...
            authdata={"identity": identity, "pubkeyid": pubkeyid},
        )
    ...
    def X509(cls, x509certificate: str) -> EntityAuthentication:
        ...
        return cls(
            ...
            authdata={"x509certificate": x509certificate},
        )
    ...
    def NPTicket(cls, npticket: bytes) -> EntityAuthentication:
        ...
        return cls(
            ...
            authdata={"npticket": base64.standard_b64encode(npticket).decode("utf-8")},
        )
    ...
    def Widevine(cls, devtype: str, keyrequest: str) -> EntityAuthentication:
        ...
        return cls(
            ...
            authdata={"devtype": devtype, "keyrequest": keyrequest},
        )
    ...
    @classmethod
    def MasterTokenProtected(
        cls, mastertoken, authdata: bytes, signature: bytes
    ) -> EntityAuthentication:
        """

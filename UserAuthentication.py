...
from widevinely.utils.MSL.schemes import UserAuthenticationSchemes
from widevinely.utils import logger

log = logger.getLogger("MSL.UserAuthentication")
...
class UserAuthentication(MSLObject):
    ...
    @classmethod
    def EmailPasswordHash(
        cls, email: str, hash_: bytes, nonce: bytes
    ) -> UserAuthentication:
        """
    ...
    def MDX(
        ...
        if mastertoken and cticket:
            ValueError
            log.exit("Only mastertoken or cticket may be set, not both")
        if mastertoken:
            authdata["mastertoken"] = mastertoken
        elif cticket:
            authdata["cticket"] = cticket
        return cls(scheme=UserAuthenticationSchemes.MDX, authdata=authdata)

    @classmethod
    def NetflixIDCookies(
        cls, netflixid: str, securenetflixid: str
    ) -> UserAuthentication:
        """
    ...
    def SSO(
        ...
        netflixid: Optional[str] = None,
        securenetflixid: Optional[str] = None,
    ) -> UserAuthentication:
        """
        ...
        if (email or password) and (netflixid or securenetflixid):
            ValueError
            log.exit(
                "Only email & password or netflixid & securenetflixid may be set, not both"
            )
        ...
        return cls(scheme=UserAuthenticationSchemes.SSO, authdata=authdata)
    ...
    def UserIDTokens(cls, mastertoken: dict, useridtoken: dict) -> UserAuthentication:
        ...
        return cls(
            ...
            authdata={"mastertoken": mastertoken, "useridtoken": useridtoken},
        )

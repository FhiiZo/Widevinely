...
class Device:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, device_code, device_key):
        ...
        if len(self.device_code) != 3:
            ValueError
            log.exit("Invalid device code length")

        if len(self.device_key) != 16:
            ValueError
            log.exit("Invalid device key length")

    def __repr__(self):
        return "<Device device_code={}, device_key={}>".format(
            self.device_code, base64.b64encode(self.device_key).decode("utf-8")
        )
...
class HuluClient(pyhulu.HuluClient):
    ...
    def get_session_key(self):
        ...
        for device_byte, server_byte in zip(
            self.device.device_key, bytes.fromhex(config["key"])
        ):
